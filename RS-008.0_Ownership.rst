   ...
   * Rust-analyzer show us this error 
     ...
     * This means you took the value that was origanally assigned in line 3, str1, 
       ...
       * (tbr) By default, for heap allocated elements rust does not do a deep 
         copy, the data on the heap will not be copied, but rather a shallow 
         copy. The pointer str2 (stack element) will point to the same heap object 
         that str1 (a stack element) points to. 
       ...
       * after line 4 ``let str2 = str1;`` Rust consider ``str1`` as no longer 
         ...
         str1 and str2. If Rust did not do this, when ``str1`` and ``str2`` goes 
         out of scope, they will both try to free the same memory location 
         and this would have lead to double free error as that would try to free t
         he same heap memory location which could lead to memory corruption.
